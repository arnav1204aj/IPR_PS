...
def evalsurfaceNormal(N_gt, N_est, mask,data):
    """
    ...
    if data=='real':
     ae, MAE = evaluate_angular_error2(gt, est)  
    if data=='irls':
     ae, MAE = evaluate_angular_error3(gt, est)        
...
def evaluate_angular_error2(gtnormal=None, normal=None, background=None):
    """
    gtnormal: [N, 3]
    normal: estimated normal [N, 3]
    background: bool index with length N
...
def evaluate_angular_error3(gtnormal=None, normal=None, background=None):
    """
    gtnormal: [N, 3]
    normal: estimated normal [N, 3]
    background: bool index with length N
